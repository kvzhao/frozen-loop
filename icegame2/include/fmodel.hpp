/* FModel: Square ice (F-model) on checkerboard lattice.
...
/* TODOs
  ...
  F-Model: (TODO!)
  * One important thing is previous sanity check should be legal.
  * Energy function (all functions related to energy computation should be modified.)
  * Remove legacy codes.
...
class FmodelGame {
    public:
        ...
        // Use Move, instead of Draw (LEGACY Code)
        ...
        */
        ...
        inline double GetSymmetricVertexDensity() {
    ...
    private:
        ...
        /* Coupling constants:
          In F model, this plays the important role.
        */
        double J1;
        double J2;
...
BOOST_PYTHON_MODULE(fmodelgame)
{
    ...
    class_<FmodelGame>("FmodelGame", init<INFO>())
        // Monte Carlo related
        .def("init_model", &FmodelGame::InitModel)
        .def("set_temperature", &FmodelGame::SetTemperature)
        .def("mc_run", &FmodelGame::MCRun)
        .def("long_loop_algorithm", &FmodelGame::LongLoopAlgorithm)

        // System commands
        .def("start", &FmodelGame::Start)
        .def("reset", &FmodelGame::Reset)
        .def("reset_config", &FmodelGame::ResetConfiguration)
        .def("timeout", &FmodelGame::TimeOut)
        .def("print_lattice", &FmodelGame::PrintLattice)
        .def("clear_buffer", &FmodelGame::ClearBuffer)

        // REVISE, change the state
        .def("move", &FmodelGame::Move)
        .def("flip", &FmodelGame::Flip)
        .def("guide_action", &FmodelGame::GuideAction)

        .def("metropolis", &FmodelGame::Metropolis)
        .def("init_agent_site", &FmodelGame::InitAgentSite)
        .def("flip_trajectory", &FmodelGame::FlipTrajectory)
        .def("follow_trajectory", &FmodelGame::FollowTrajectory)
        .def("update_config", &FmodelGame::UpdateConfig)
        .def("set_ice", &FmodelGame::SetIce)

        // State Raw data
        .def("get_state_t", &FmodelGame::GetStateT)
        .def("get_state_tp1", &FmodelGame::GetStateTp1)
        .def("get_state_diff", &FmodelGame::GetStateDiff)

        // Observations

        // Updated
        .def("get_agent_site", &FmodelGame::GetAgentSite)
        .def("get_agent_init_site", &FmodelGame::GetAgentInitSite)
        .def("get_agent_spin", &FmodelGame::GetAgentSpin)
        .def("get_state_t_map", &FmodelGame::GetStateTMap)
        .def("get_state_tp1_map", &FmodelGame::GetStateTp1Map)
        .def("get_state_diff_map", &FmodelGame::GetStateDiffMap)

        .def("get_neighbor_spins", &FmodelGame::GetNeighborSpins)
        .def("get_neighbor_sites", &FmodelGame::GetNeighborSites)

        .def("get_phy_observables", &FmodelGame::GetPhyObservables)
        .def("get_symmetric_vertex_density", &FmodelGame::GetSymmetricVertexDensity)
        ...
        .def("get_episode", &FmodelGame::GetEpisode)
        .def("get_ep_step_counter", &FmodelGame::GetEpStepCounter)
        .def("get_total_acceptance_rate", &FmodelGame::GetTotalAcceptanceRate)
        .def("get_action_statistics", &FmodelGame::GetActionStatistics)
        .def("get_ep_action_counters", &FmodelGame::GetEpActionCounters)
        .def("get_ep_action_list", &FmodelGame::GetEpActionList)
        .def("get_updated_counter", &FmodelGame::GetUpdatedCounter)
        .def("get_updating_counter", &FmodelGame::GetUpdatingCounter)
        .def("get_accepted_length", &FmodelGame::GetAcceptedLen)
        .def("get_trajectory", &FmodelGame::GetTrajectory)
    ;
}