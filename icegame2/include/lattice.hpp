...
class Checkerboard : public Lattice{
  // New added class for four sublattice checkerboard.
  public:
    Checkerboard();
    //Checkerboard(int, int&);    
    ~Checkerboard();
    void init_from_size(int lattice_size); 
    void init(INFO info);
    void construct_lattice();
    void find_neighbors();
    void neighbors1();
    ...
    void output_lattice();
    int periodic_bd(int, int);
    int get_neighbor_shift_bd(int, int, int, int);
};
