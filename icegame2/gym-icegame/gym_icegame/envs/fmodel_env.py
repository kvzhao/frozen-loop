...
import random
import numpy as np
from fmodelgame import FmodelGame, INFO
...
class FModelGameEnv(core.Env):
    def __init__ (self, L, kT, 
        ...
        self.sim = FmodelGame(self.mc_info)
    ...
    def auto_step(self):
        ...
        # NOTE: Dont Trust auto_step in F-Model.
    ...
    def step(self, action):
        ...
        """ Results from f model
            index 0 plays two roles:
                if action is walk:
                    ...
                    rets[0] = is_accept
        """
        is_accept, boltzman_weight, Energy, dEnergy, dConfig = rets
        ...
        if (metropolis_executed):
            ...
            if is_accept > 0 and dConfig > 0:
                ...
                print ("\tAcceptance ratio (from fmodel) = {}%".format(total_acc_rate))
    ...
    def show_info(self):
        ...
        print ("Energy: {}, Vertex Density: {}".format(self.physical_observables[0],
            self.sim.get_symmetric_vertex_density()))
        print ("Env Settings:")
