...
void Checkerboard::init(INFO info) {
  int L = info.lattice_size;
  init_from_size(L);
} 

void Checkerboard::init_from_size(int lattice_size) {
  name = "checkerboard";
  L = lattice_size;
  N = L * L * 4; // number of sites
  construct_lattice();
  find_neighbors();
  // output_lattice();
  cout << " ------- Lattice: two dimensional checkerboard lattice\n";
}
...
int Checkerboard::periodic_bd(int p, int d){
  int result = ( (p + d) % L + L) % L;
  return result;
}

void Checkerboard::construct_lattice(){

//      lattice indices (site3d)    sublattice indices            site indices (site_index3d)	
//
//	(0,1) ----- (1,1) ...	      (3) ----- (2) ...        ...       ...       ...
//        |	      |		       |	 |	        |	  |         |
//	  |	      | 	       |	 |	        |	  |	    |
//	  |	      | 	       |	 |	        |	  |	    |
//	(0,0) ----- (1,0) ...	      (0) ----- (1) ...       (3 - 2) ----- (7 - 6) ...(4L-1 4L-2)
//                                                             |   |         |   |       |     |
//                                                            (0 - 1) ----- (4 - 5) ...(4L-4 4L-3)
  ...
  site_index3d.resize(L); //site_index3d[L][L][4]
  vtr3d::iterator it3d;
  vtr2d::iterator it2d;
  for (it3d = site_index3d.begin(); it3d != site_index3d.end(); it3d++){
    (*it3d).resize(L);
  ...
  Coordinates2d Bravais_a, Bravais_b;
  Bravais_a.x = 1.0; Bravais_a.y = 0.0;
  Bravais_b.x = 0.0; Bravais_b.y = 1.0;
...
int Checkerboard::get_neighbor_shift_bd(int p, int dx, int dy, int p2_sub){
  return 0;
}

void Checkerboard::find_neighbors(){
      neighbors1();
}

void Checkerboard::neighbors1(){
  for (int i = 0; i < N; i++){
    NN.push_back(vector<int> (6));
  }
  ...
  for (int p = 0; p < N; p++){
  	...
  	int x = site3d[p].x;
  	int y = site3d[p].y;
  	int xp = periodic_bd(x, 1);
  	int xm = periodic_bd(x, -1);
  	int yp = periodic_bd(y, 1);
  	int ym = periodic_bd(y, -1);
  	switch(sub[p]) {
  	  case 0:{
  		  NN[p][0] = site_index3d[x][y][1];
  		  NN[p][1] = site_index3d[x][y][2];
  		  NN[p][2] = site_index3d[x][y][3];
  		  NN[p][3] = site_index3d[xm][y][1];
  		  NN[p][4] = site_index3d[xm][ym][2];
  		  NN[p][5] = site_index3d[x][ym][3];
  		  break;
  		}
  	  case 1:{
  		  NN[p][0] = site_index3d[x][y][2];
  		  NN[p][1] = site_index3d[x][y][3];
  		  NN[p][2] = site_index3d[x][y][0];
  		  NN[p][3] = site_index3d[x][ym][2];
  		  NN[p][4] = site_index3d[xp][ym][3];
  		  NN[p][5] = site_index3d[xp][y][0];
  		  break;
  		}
  	  case 2:{
  		  NN[p][0] = site_index3d[x][y][3];
		  NN[p][1] = site_index3d[x][y][0];
		  NN[p][2] = site_index3d[x][y][1];
		  NN[p][3] = site_index3d[xp][y][3];
		  NN[p][4] = site_index3d[xp][yp][0];
		  NN[p][5] = site_index3d[x][yp][1];
		  break; 
		}
	  case 3:{
		  NN[p][0] = site_index3d[x][y][0];
		  NN[p][1] = site_index3d[x][y][1];
		  NN[p][2] = site_index3d[x][y][2];
		  NN[p][3] = site_index3d[x][yp][0];
		  NN[p][4] = site_index3d[xm][yp][1];
		  NN[p][5] = site_index3d[xm][y][2];
		  break; 
		} } // end of switch
  } // end of loop
}

void Checkerboard::show_lattice(){
	cout << "Show all the lattice sites of size L = " << L << " system." << endl;
	for (int i = 0; i < N; i++){
		cout << " site[" << i << "] = " << "(" << site3d[i].x << "," << site3d[i].y << "), "
		    << " sublattice = " << sub[i] << endl;
...
void Checkerboard::output_lattice() {
  // no implementation.
}