...
FmodelGame::FmodelGame (INFO info) : sim_info(info) {
    model.init(sim_info);
    latt.init(sim_info);
    ...
    // Serious Problem ====
    // How to set coupling constant?
    // void Hamiltonian::insert_Eng(double Ji) this function pushes 
    //  Ji into vector<double> J and using Eng() pointer
    J1 = 1.0;
    J2 = -0.05;
    model.set_J1(J1);
    model.set_J2(J2);
    std::cout << "[GAME] Set Coupling J1: " 
            << J1 << ", J2: " << J2 << "\n"; 
    ...
    std::cout << "[GAME] FmodelGame is created.\n";
}

bool FmodelGame::TimeOut() {
...
void FmodelGame::update_state_to_config() {
    ...
    if (_cal_energy_density_of_state(ice_config.Ising) <= AVERAGE_GORUND_STATE_ENERGY) {
        // Now, the system can have <E> <= -1.0 (~-1.05)
        // Silence here!
...
double FmodelGame::SetIce(const boost::python::object &iter) {
...
void FmodelGame::MCRun(int mcSteps) {
    /*
        Prepare an ice state for game
    */
    tt.timer_begin();
    for (int i = 0; i < mcSteps; ++i) {
        model.MCstep_ICE(&ice_config, &latt);
    }
    tt.timer_end();

    //std::cout << "[GAME] Monte Carlo runs " << mcSteps << " steps with " << tt.timer_duration() << " seconds.\n"; 
    
    // Check whether it is icestates
    double Etot = model.total_energy(&ice_config, &latt);
    //std::cout << "[GAME] Total Energy E = " << Etot << " (model).\n";
    std::cout << "[GAME] MCRun reaches Energy <E> = " << Etot / N << " (model).\n";
...
int FmodelGame::Start(int init_site) {
...
void FmodelGame::ClearBuffer() {
    clear_all();
    same_ep_counter++;
}

// Reset: Clear buffer and 
int FmodelGame::Reset(int init_site) {
    ClearBuffer();
    Start(init_site);
    same_ep_counter = 0;
    num_episode++;
...
// member functions 
vector<double> FmodelGame::Metropolis() {
    /* Metropolis operation called by agent.
      ...
      TODOs:
        * IT IS CRITICAL THAT ACCEPTANCE SHOULD BE DETERMINED BY METROPOLIS ALGORITHM.
        * How to design the f model game?
    */
    vector<double> rets;
    ...
    double boltzman_weight = exp(-dE/kT);

    // Metropolis Criterion
    if (dice < boltzman_weight) {
        std::cout << "Accept by metropolis (dice = " << dice
                  << ", weight = " << boltzman_weight
                  << ", E = " << Et
                  << ", dE = " << dE << ")\n";
        is_accept = true;
        ...
        // Q: Do we need restore?
        std::cout << "Reject by metropolis (dice = " << dice
                  << ", weight = " << boltzman_weight
                  << ", dE = " << dE << ")\n";
        rets.emplace_back(REJECT_VALUE);
    }

    rets.emplace_back(boltzman_weight);
    rets.emplace_back(Et);
...
int FmodelGame::_flip_state_tp1_site(int site) {
...
int FmodelGame::_flip_state_t_site(int site) {
...
// LEGACY
void FmodelGame::flip_agent_state_tp1() {
...
void FmodelGame::flip_along_trajectory(const vector<int>& traj) {
...
void FmodelGame::FollowTrajectory(const boost::python::object &iter) {
...
void FmodelGame::push_site_to_trajectory(int site) {
...
int FmodelGame::get_agent_site() {
    return agent_site;
}

int FmodelGame::get_spin(int site) {
...
int FmodelGame::get_agent_spin() {
    return get_spin(agent_site);
}

// GRAY ZONE.

vector<double> FmodelGame::Move(int dir_idx) {
    // Move and Flip?
    vector<double> rets;
...
int FmodelGame::put_agent(int new_site) {
...
int FmodelGame::put_and_flip_agent(int site) {
...
ActDir FmodelGame::how_to_go(int site) {
    // Direction --> Site Mapping
    ActDir dir = get_direction_by_sites(agent_site, site);
    return dir;
}

vector<double> FmodelGame::GetPhyObservables() {
...
vector<int> FmodelGame::get_neighbor_spins() {
...
vector<int> FmodelGame::_get_neighbor_of_site(int _site) {
...
vector<int> FmodelGame::_get_neighbor_of_index(int index) {
...
int FmodelGame::get_site_by_direction(int dir_idx) {
    int site = agent_site;
...
/*
    Move: directional index is designed as
      ...
      * tail dir
*/
ActDir FmodelGame::get_direction_by_sites(int site, int next_site) {
...
vector<int> FmodelGame::GetStateTMap() {
...
vector<int> FmodelGame::GetStateTp1Map() {
...
vector<int> FmodelGame::GetStateDiffMap() {
...
vector<int> FmodelGame::GetStateDiff() {
...
double FmodelGame::_cal_energy_of_state(const vector<int> & s) {
    double eng = 0;
    for (int i = 0; i < N; i++) {
        double se = 0;
        double fe = 0;
        ...
        se *= s[i];
        int head = s[i]+s[latt.NN[i][0]]+s[latt.NN[i][1]]+s[latt.NN[i][2]];
        int tail = s[i]+s[latt.NN[i][3]]+s[latt.NN[i][4]]+s[latt.NN[i][5]];
        // Find specific (symmetric vertex) pattern.
        if (0 == head) {
            if (s[i] == s[latt.NN[i][1]]) {
                fe += 1;
            }
        }
        if (0 == tail) {
            if (s[i] == s[latt.NN[i][4]]) {
                fe += 1;
            }
        }

        // Sum the local energy (cal_Eng())
        eng += J1 * se + J2 * fe;
    }
    eng = eng / 2;
    return eng;
...
double FmodelGame::_cal_energy_density_of_state(const vector<int> & s) {
    double eng = 0.0;
    for (int i = 0; i < N; i++) {
        double se = 0.0;
        double fe = 0.0;
        ...
        // int head = _icerule_head_check(_index_to_site1d(i));
        // int tail = _icerule_tail_check(_index_to_site1d(i));
        int head = s[i]+s[latt.NN[i][0]]+s[latt.NN[i][1]]+s[latt.NN[i][2]];
        int tail = s[i]+s[latt.NN[i][3]]+s[latt.NN[i][4]]+s[latt.NN[i][5]];
        // Find specific (symmetric vertex) pattern.
        if (0 == head) {
...
int FmodelGame::_cal_defect_number_of_state(const vector<int> &s) {
    int num_defects = 0;
    for (int i = 0 ; i < N; i++) {
        //if (0 == latt.sub[i]) {
            int n0 = latt.NN[i][0];
...
double FmodelGame::_cal_symmetric_vertex_density_of_state(const vector<int> &state) {
    int num_defects = 0;
    for (int i = 0; i < N; ++i) {
...
double FmodelGame::_cal_defect_density_of_state(const vector<int> & s) {
...
double FmodelGame::_cal_mean(const vector<int> &s) {
    return 1.0* std::accumulate(s.begin(), s.end(), 0LL) / s.size();
}

double FmodelGame::_cal_stdev(const vector<int> &s) {
...
template <class T>
void FmodelGame::_print_vector(const vector<T> &v, const std::string &prefix) {
    std::cout << prefix << ": [";
    for (const auto &i: v)
        std::cout << i << ", ";
    std::cout << "]\n";
...
bool FmodelGame::_is_visited(int site) {
    bool visited = false;
...
bool FmodelGame::_is_start_end_meets(int site) {
    return (site == init_agent_site) ? true : false;
}

bool FmodelGame::_is_traj_continuous() {
    // LEGACY CODE
    bool cont = true;
...
bool FmodelGame::_is_traj_intersect() {
    /*
        Not complete
    */
    bool meet = false;
...
int FmodelGame::_count_config_difference(const vector<int> &c1, const vector<int> &c2) {
...
vector<int> FmodelGame::LongLoopAlgorithm() {
...
vector<int> FmodelGame::GuideAction() {
    //Args:
...
int FmodelGame::_loop_extention(int curr_site){
...
int FmodelGame::_icerule_head_check(int site) {
...
int FmodelGame::_icerule_tail_check(int site) {
...
vector<int> FmodelGame::_end_sites(int site) {
...
vector<int> FmodelGame::_indices_to_sites1d(const vector<int> indices) {
    vector<int> site1d;
...
vector<int> FmodelGame::_sites1d_to_indices(const vector<int> sites) {
    vector<int> indices;
...
void FmodelGame::show_information() {
    // Game information
