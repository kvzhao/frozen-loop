...
void Ising::init(INFO info) {
  ...
  J3a = 0;
  J3b = 0;
  cout << " ------- Hamiltonian: Ising model with layer of neighbors = " << Num_neighbors << endl;
}
...
Square_ice::Square_ice(INFO MC, int nei){
    // Why model (hamiltonian) dont know the lattice type?
    // Well,... that's make scence i think.
      L = MC.lattice_size;
      N = MC.Num_sites;
      R = MC.Num_replicas;
...
// This function should be updated if we using Checkerboard.
...
bool Square_ice::SSF(Sample* ss, Lattice* la){
      int random = (uni01_sampler() * N);
      // double delta_m = (-2.0) * ss->Ising[random];
      // double delta = (-2.0) * cal_Eng(random, ss, la) + (-2.0) * ME(random, ss, la); 
      double delta_se = (-2.0) * SE(random, ss, la);
      //double delta_me = (-2.0) * ME(random, ss);
      //double delta = delta_se + delta_me;
