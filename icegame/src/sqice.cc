...
void SQIceGame::SetIce(vector<double> config) {
    // TODO
} 
...
vector<int> SQIceGame::LongLoopAlgorithm() {
    ...
    bool is_closed = true;
    // It seems we have to create the first dipole?
    //Flip();
    //trajectory.push_back(start_site);

    // Loop for long loop
    while (true) {
        int spin = get_agent_spin();
        int site = get_agent_site();
        std::cout << "Agent spin is " << spin << " with site: " << site <<  "\n";
        // get the same spins
        vector<int> nspins = get_neighbor_candidates(true);

        std::cout << "Candidates: " << nspins[0] << "\n";

        for (auto const & s: nspins) {
            std::cout << s << " ";
        }
        std::cout << "\n";

        // should i call 'go' here? sure, we call how_to_go first

        //vector<double> obs = GetPhyObservables();
        //std::cout << " dE " << obs[0] 
        //          << " dD " << obs[1] 
        //          << " dC " << obs[2] << "\n";

        // add to loop traj (site or action index)

        int candidate_site = nspins[0];

        if (candidate_site == start_site) {
            std::cout << " meet the staring site: " << start_site << "\n";
            is_closed = true;
        } else {
            bool is_visited = false;
            if (candidate_site == trajectory.back()) {
                is_visited = true;
            }
            /*
        ...
        int idx = how_to_go(candidate_site);
        std::cout << " action idx : " << idx << "\n";
        int agent_new_site = go(idx);
        std::cout << "New, Agent spin is " << get_agent_spin() << " with site: " << agent_new_site <<  "\n";
        trajectory.push_back(site);
