...
def create_commands(session, num_workers, remotes, policy, env_id, logdir, shell='bash', mode='tmux', visualise=False):
    ...
    # add one line for running monitor
    pe_cmd = [
        'CUDA_VISIBLE_DEVICES=',
    ...
    cmds_map += [new_cmd(session,
                "pe", 
                pe_cmd + ["--job-name", "monitor", "--task", str(num_workers-1), "--remotes", remotes[num_workers-1]], mode, logdir, shell)]
    ...
    if mode == 'tmux':
        cmds += [
        "kill $( lsof -i:6006 -t ) > /dev/null 2>&1",  # kill any process using tensorboard's port
